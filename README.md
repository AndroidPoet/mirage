...
Mirage has two kinds of shaders:
...
| `sizing` | `SizingParams` | Fit, `scale`, `rotation`, `origin`, and `offset` of the graphic. |

Each shader also has a `<Name>Defaults` object holding its default colors, so you can reuse
or tweak the preset instead of hardcoding.
...
Presets `SizingParams.Object` (fit = Contain) and `SizingParams.Pattern` (fit = None) are
provided; use `.copy(...)` to tweak. Many shaders also expose `grainMixer` / `grainOverlay`
(0f–1f) to add film-grain texture.

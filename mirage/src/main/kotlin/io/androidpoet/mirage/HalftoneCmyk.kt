...
float colorMask(vec2 pos, vec2 cellCenter, float rad, float transparency, float grain, float channelAddon, float channelgain, float generalComp, bool isJoined) {
...
half4 main(vec2 fragCoord) {
  ...
  if (u_type < 1.5) {
    ...
    for (int dy = -1; dy <= 1; dy++) {
      for (int dx = -1; dx <= 1; dx++) {
        ...
        vec4 texC = u_image.eval(gridToImageUV(cellCenterC, cosC, sinC, shiftC, pad) * u_resolution);
        outMask.x += colorMask(uvC, cellCenterC, getCyan(texC), insideImageBox * texC.a, grain, u_floodC, u_gainC, generalComp, isJoined);

        vec2 cellCenterM = cellCenterPos(uvM, cellOffset, 1.);
        vec4 texM = u_image.eval(gridToImageUV(cellCenterM, cosM, sinM, shiftM, pad) * u_resolution);
        outMask.y += colorMask(uvM, cellCenterM, getMagenta(texM), insideImageBox * texM.a, grain, u_floodM, u_gainM, generalComp, isJoined);

        vec2 cellCenterY = cellCenterPos(uvY, cellOffset, 2.);
        vec4 texY = u_image.eval(gridToImageUV(cellCenterY, cosY, sinY, shiftY, pad) * u_resolution);
        outMask.z += colorMask(uvY, cellCenterY, getYellow(texY), insideImageBox * texY.a, grain, u_floodY, u_gainY, generalComp, isJoined);

        vec2 cellCenterK = cellCenterPos(uvK, cellOffset, 3.);
        vec4 texK = u_image.eval(gridToImageUV(cellCenterK, cosK, sinK, shiftK, pad) * u_resolution);
        outMask.w += colorMask(uvK, cellCenterK, getBlack(texK), insideImageBox * texK.a, grain, u_floodK, u_gainK, generalComp, isJoined);
      }
    }
  } else {
    ...
    for (int dy = -1; dy <= 1; dy++) {
      for (int dx = -1; dx <= 1; dx++) {
        ...
        outMask.x += colorMask(uvC, cellCenterPos(uvC, cellOffset, 0.), cmykOriginal.x, insideImageBox, grain, u_floodC, u_gainC, generalComp, isJoined);
        outMask.y += colorMask(uvM, cellCenterPos(uvM, cellOffset, 1.), cmykOriginal.y, insideImageBox, grain, u_floodM, u_gainM, generalComp, isJoined);
        outMask.z += colorMask(uvY, cellCenterPos(uvY, cellOffset, 2.), cmykOriginal.z, insideImageBox, grain, u_floodY, u_gainY, generalComp, isJoined);
        outMask.w += colorMask(uvK, cellCenterPos(uvK, cellOffset, 3.), cmykOriginal.w, insideImageBox, grain, u_floodK, u_gainK, generalComp, isJoined);
      }
    }
  }
  ...
  if (isJoined) {
    ...
    // Derivatives are unavailable in AGSL: the masks vary at cell frequency,
    // so estimate their screen-space derivative from the cell size in pixels.
    float aa = 2. * cellsPerSide / u_resolution.y;
    float th = .5;
...
/** Dot shape style for [halftoneCmyk]. */
public enum class HalftoneCmykType(public val value: Float) {
  Dots(0f),
  Ink(1f),
  Sharp(2f),
}

public object HalftoneCmykDefaults {
  public val ColorBack: Color = Color(0xFFFBFAF5)
  public val ColorC: Color = Color(0xFF00B4FF)
  public val ColorM: Color = Color(0xFFFC519F)
  public val ColorY: Color = Color(0xFFFFD800)
  public val ColorK: Color = Color(0xFF231F20)
}

/**
 * CMYK halftone printing effect applied to the content, with customizable dot
 * patterns and ink colors for each channel (Cyan, Magenta, Yellow, Black).
 */
@RequiresApi(Build.VERSION_CODES.TIRAMISU)
@Composable
public fun Modifier.halftoneCmyk(
  ...
  setColorUniform4f("u_colorC", colorC)
  setColorUniform4f("u_colorM", colorM)
  setColorUniform4f("u_colorY", colorY)
  setColorUniform4f("u_colorK", colorK)
  ...
  setFloatUniform("u_gridNoise", gridNoise)
  setFloatUniform("u_floodC", floodC)
  setFloatUniform("u_floodM", floodM)
  setFloatUniform("u_floodY", floodY)
  setFloatUniform("u_floodK", floodK)
  setFloatUniform("u_gainC", gainC)
  setFloatUniform("u_gainM", gainM)
  setFloatUniform("u_gainY", gainY)
  setFloatUniform("u_gainK", gainK)
